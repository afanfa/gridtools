...
void m_packZL(array_t const &d_data_array,
    ...
    for (int i = 0; i < niter; i++) {
        ...
        hipLaunchKernelGGL(m_packZLKernel, dim3(blocks), dim3(threads), 0, 0, d_data_array[i], d_msgbufTab, d_msgsize, halo_d, nx, ny, i);
...
int call_kernel_ZL(Blocks blocks,
    ...
    int ny,
    unsigned int i) {
    hipLaunchKernelGGL(m_packZLKernel, dim3(blocks), dim3(threads), b, 0, d_data, d_msgbufTab, d_msgsize, halo_d, nx, ny, i);
