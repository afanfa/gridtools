...
void m_unpackYL(array_t const &d_data_array,
    ...
    for (int i = 0; i < niter; i++) {
        ...
        hipLaunchKernelGGL(m_unpackYLKernel, dim3(blocks), dim3(threads), 0, 0, d_data_array[i],
            d_msgbufTab_r,
            d_msgsize_r,
            halo_d,
            nx,
...
template <typename Blocks,
    ...
    typename Msgsize,
    typename Halo>
int call_kernel_YL_u(Blocks blocks,
    Threads threads,
    Bytes b,
    Pointer d_data,
    ...
    int i) {
    hipLaunchKernelGGL(m_unpackYLKernel, dim3(blocks), dim3(threads), b, 0, d_data, d_msgbufTab, d_msgsize, halo_d, nx, ny, tranlation_const, i);
...
void m_unpackYL_variadic(value_type **d_msgbufTab_r,
    int d_msgsize_r[27],
    const gridtools::halo_descriptor halo[3],
    const gridtools::halo_descriptor halo_d[3],
    const datas &d_datas,
    ...
    int nothing[niter] = {call_kernel_YL_u(blocks,
        threads,
        0,
